...
import { generateXPGraph, generateAuditRatioPieChart, generateProjectSuccessChart, generateSkillsRadarChart } from "./graphs.js";
...
function updateUI(done) {
  ...
  // Populate additional charts with error handling
  try {
    const projectSuccessChart = document.getElementById("project-success-chart");
    const auditRatioChart = document.getElementById("audit-ratio-chart");
    const skillsRadarChart = document.getElementById("skills-radar-chart");

    if (projectSuccessChart) {
      projectSuccessChart.innerHTML = '<div class="loading">Loading chart...</div>';
      setTimeout(() => {
        projectSuccessChart.innerHTML = generateProjectSuccessChart({
          go: done.go,
          javascript: done.js,
          rust: done.rust
        });
    ...
    if (auditRatioChart) {
      auditRatioChart.innerHTML = '<div class="loading">Loading chart...</div>';
      setTimeout(() => {
        // Calculate actual audit success/failure from transactions
        const totalUp = currentUser.transactions.reduce((total, transaction) => {
          return transaction.type === "up" ? total + transaction.amount : total;
        }, 0);
        const totalDown = currentUser.transactions.reduce((total, transaction) => {
          return transaction.type === "down" ? total + transaction.amount : total;
        }, 0);

        // Calculate the actual ratio (up / (up + down))
        const totalAudits = totalUp + totalDown;
        const actualAuditRatio = totalAudits > 0 ? totalUp / totalAudits : 0;
        ...
        console.log('Audit Data:', {
          ...
          totalAudits,
          actualAuditRatio,
          originalAuditRatio: currentUser.auditRatio
        });

        auditRatioChart.innerHTML = generateAuditRatioPieChart(actualAuditRatio, totalUp, totalDown);
      }, 200);
    }

    if (skillsRadarChart) {
      skillsRadarChart.innerHTML = '<div class="loading">Loading chart...</div>';
      setTimeout(() => {
        const topSkills = skillTypes
        ...
        skillsRadarChart.innerHTML = generateSkillsRadarChart(topSkills);
      }, 300);
    }
  } catch (error) {
...
function setupChartNavigation() {
  ...
  const chartContainers = document.querySelectorAll('.additional-charts .chart-container');
  ...
  navButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      chartContainers.forEach(container => {
        ...
        } else if (container.classList.contains(`chart-${chartType}`)) {
          container.classList.remove('hidden');
        } else {
